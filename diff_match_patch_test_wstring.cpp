 ...
 *
 * Diff Match and Patch -- Test Harness using std::wstring
...
class diff_match_patch_test : diff_match_patch<wastring> {
 ...
 public:
  ...
  void testDiffDelta() {
    ...
    diffs = diffList(Diff(EQUAL, string_t(L"\u0680 \000 \t %", 7)), Diff(DELETE, string_t(L"\u0681 \001 \n ^", 7)), Diff(INSERT, string_t(L"\u0682 \002 \\ |", 7)));
 ...
 private:
  ...
  Strings diff_halfMatch(const string_t &text1, const string_t &text2) {
    Strings list;
    HalfMatchResult hm;
    if (diff_match_patch<wastring>::diff_halfMatch(text1, text2, hm)) {
